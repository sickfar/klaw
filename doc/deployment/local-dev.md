...
Build from source and run Engine + Gateway in Docker. Both communicate via TCP (port `7470`) over the Docker network. Containers run as the non-root `klaw` user (UID 10001).

> **Important:** There is no `docker-compose.yml` in the repository root. Use `klaw init` to generate a compose file, or use the production compose file at `docker/compose/docker-compose.prod.json`.

## Setup

### 1. Build artifacts and Docker images

```bash
./scripts/build.sh
```

This runs `./gradlew assembleDist` and then builds Docker images (engine, gateway, sandbox, and cli if on Linux). The Dockerfiles COPY from `build/dist/` — artifacts must exist before building images.

> **Do NOT use `docker compose build`** — there is no compose build file. Always use `./scripts/build.sh`.

### 2. Create config and run init

Run `klaw init` to generate config files and start services:

```bash
# If you have the native CLI binary:
./build/dist/klaw-macosArm64 init   # macOS
./build/dist/klaw-linuxX64 init     # Linux

# Or via Docker (generates compose file in klaw-config volume):
docker run -it --rm \
...
The `./klaw` wrapper runs `docker compose run --rm cli "$@"` — each invocation creates a fresh container and removes it on exit. The wrapper requires a compose file generated by `klaw init`.
...
| Config | `./config/*.json` (bind-mount) | `~/.config/klaw/*.json` |
