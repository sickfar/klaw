...
class CliCommandDispatcher(
    ...
    suspend fun dispatch(request: CliRequestMessage): String =
        withContext(Dispatchers.VT) {
            when (request.command) {
                "klaw_init_status" -> initCliHandler.handleStatus()
                "klaw_init_generate_identity" -> initCliHandler.handleGenerateIdentity(request.params)
                "status" -> handleStatus()
                ...
                "memory_search" -> handleMemorySearch(request.params)
                "reindex" -> handleReindex()
                else -> """{"error":"unknown command: ${request.command.replace("\\", "\\\\").replace("\"", "\\\"")}"}"""
            }
        }
    ...
    private suspend fun handleMemorySearch(params: Map<String, String>): String {
        ...
        val topK = params["top_k"]?.toIntOrNull() ?: 10
