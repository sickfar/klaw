...
6. Whitelist проверка исходящих (channel + chatId из `gateway.yaml`)
...
9. `setMyCommands` в Telegram при старте (регистрация bot-команд из engine.yaml)
...
- **Location** — `~/.config/klaw/gateway.yaml`; read-only from the agent's perspective (outside workspace); the user must edit this file to change settings
...
- **What to do when send_message fails** — check the tool result error message; if `"chatId not in allowedChatIds"`, the target must be added to `gateway.yaml` by the user; the agent cannot modify this file
...
- **Configuration** — `~/.config/klaw/gateway.yaml` (channels, tokens, allowedChatIds); `~/.config/klaw/engine.yaml` (LLM providers, models, routing, memory, scheduling, code execution)
