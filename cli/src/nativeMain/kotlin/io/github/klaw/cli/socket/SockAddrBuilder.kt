 ...
 * - macOS: { uint8_t sin_len=16, uint8_t sin_family=AF_INET(2), uint16_t sin_port(BE),
 *            in_addr(4B), zero(8B) } = 16 bytes
 * - Linux: { uint16_t sin_family=AF_INET(2) LE, uint16_t sin_port(BE),
 *            in_addr(4B), zero(8B) } = 16 bytes
 */
...
internal fun parseIpv4(host: String): ByteArray {
    ...
    require(parts.size == IPV4_PARTS) { "Invalid IPv4 address: $host" }
    return ByteArray(IPV4_BYTES) { i ->
        val octet = parts[i].toIntOrNull() ?: error("Invalid IPv4 octet '${parts[i]}' in: $host")
        require(octet in 0..IPV4_OCTET_MAX) { "IPv4 octet out of range (0-255): $octet in $host" }
        octet.toByte()
    }
}
