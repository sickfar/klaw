...
class EngineCommandRegistryTest {
    private fun mockCommand(name: String, description: String = "desc") = mockk<EngineSlashCommand> {
        every { this@mockk.name } returns name
        every { this@mockk.description } returns description
    }

    private fun makeConfig(commands: List<CommandConfig> = emptyList()) = mockk<EngineConfig> {
    ...
    fun `find returns command by name`() {
        ...
        val registry = EngineCommandRegistry(
