...
class CommandHandlerTest {
    ...
    @Test
    fun `delegates to registered command`() = runTest {
        val mockCommand = mockk<EngineSlashCommand>(relaxed = true) {
            coEvery { handle(any(), any()) } returns "Command executed"
        }
        val registry = mockk<EngineCommandRegistry> {
    ...
    @Test
    fun `returns unknown command message for unregistered command`() = runTest {
        val registry = mockk<EngineCommandRegistry> {
