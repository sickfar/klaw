...
**Никакой компакции.** Контекст не мутируется — он пересобирается каждый раз. Conversation log растёт бесконечно (append-only JSONL). Старые сообщения не удаляются, а «покидают» скользящее окно.
...
### 3.4. Фоновая суммаризация

Вместо компакции OpenClaw (где LLM сжимает историю и теряет детали):

1. Scheduler каждые 50 сообщений (или по cron) генерирует задачу `summarize_old_messages`
2. Engine берёт блок старых сообщений, ещё не покрытых саммари
3. Вызывает LLM (можно дешёвую модель) для создания краткого саммари
4. Саммари сохраняется как Markdown-файл и индексируется в sqlite-vec
5. Оригинальные сообщения **остаются в логе навсегда**
