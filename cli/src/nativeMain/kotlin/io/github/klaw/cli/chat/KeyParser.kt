...
internal class KeyParser {
    ...
    fun feed(byte: Int): ChatEvent? =
        when (state) {
            STATE_NORMAL -> feedNormal(byte)
            STATE_AFTER_ESC -> feedAfterEsc(byte)
            STATE_AFTER_CSI -> feedAfterCsi(byte)
            STATE_AFTER_CSI3 -> feedAfterCsi3(byte)
            STATE_UTF8 -> feedUtf8(byte)
            else -> {
                state = STATE_NORMAL
                null
            }
    ...
    private fun feedNormal(byte: Int): ChatEvent? =
        when (byte) {
            ...
            CTRL_C -> ChatEvent.Quit
            DEL -> ChatEvent.Backspace
            CR, LF -> ChatEvent.Enter
            in SPACE..ASCII_END -> ChatEvent.KeyPressed(byte.toChar().toString())
            in UTF8_2BYTE_MIN..UTF8_2BYTE_MAX -> startUtf8(byte, 2)
            in UTF8_3BYTE_MIN..UTF8_3BYTE_MAX -> startUtf8(byte, UTF8_3BYTE_LEN)
            in UTF8_4BYTE_MIN..UTF8_4BYTE_MAX -> startUtf8(byte, UTF8_MAX_BYTES)
            else -> null
        }
    ...
    private fun feedAfterCsi(byte: Int): ChatEvent? {
        state = STATE_NORMAL
        return when (byte) {
            CSI_UP -> ChatEvent.ArrowKey(ChatEvent.ArrowKey.Direction.UP)
            CSI_DOWN -> ChatEvent.ArrowKey(ChatEvent.ArrowKey.Direction.DOWN)
            CSI_RIGHT -> ChatEvent.ArrowKey(ChatEvent.ArrowKey.Direction.RIGHT)
            CSI_LEFT -> ChatEvent.ArrowKey(ChatEvent.ArrowKey.Direction.LEFT)
            CSI_HOME -> ChatEvent.Home
            CSI_END -> ChatEvent.End
            CSI_DEL_INTRO -> {
