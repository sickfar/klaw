        ...
        """
{
  ...
  "processing": {"debounceMs": 100, "maxConcurrentLlm": 1, "maxToolCallRounds": 1}${if (consolidationBlock.isNotEmpty()) ",\n  $consolidationBlock" else ""}
}
        """.trimIndent()
    ...
    fun `parse consolidation with all fields`() {
        ...
        assertTrue(config.consolidation.enabled)
        assertEquals("0 30 3 * * ?", config.consolidation.cron)
        assertEquals("glm/glm-5", config.consolidation.model)
        assertEquals(listOf("telegram_123", "discord_456"), config.consolidation.excludeChannels)
        assertEquals("daily-notes", config.consolidation.category)
        assertEquals(10, config.consolidation.minMessages)
    }
    ...
    fun `parse consolidation with empty section uses defaults`() {
        ...
        val config = parseEngineConfig(json)
        assertFalse(config.consolidation.enabled)
        assertEquals("0 0 0 * * ?", config.consolidation.cron)
        assertEquals("", config.consolidation.model)
        assertTrue(config.consolidation.excludeChannels.isEmpty())
        assertEquals("daily-summary", config.consolidation.category)
        assertEquals(5, config.consolidation.minMessages)
    }
